...
N_NEURONS = 128
# architecture
class Nps(torch.nn.Module):
    def __init__(self,input_size=3, output_size=14):
        super().__init__()
        self.linear1 = torch.nn.Linear(input_size,N_NEURONS)  
        self.linear2 = torch.nn.Linear(N_NEURONS, N_NEURONS)  
        self.linear3 = torch.nn.Linear(N_NEURONS, N_NEURONS)   
    ...
    def forward(self, array):
        ...
        res = self.activation(self.linear6(res))
        res = (self.output(res))
        z_mean = torch.nn.functional.tanh(res[:,:7]) * 0.5
        z_log_var = torch.nn.functional.sigmoid(res[:,7:]) * -15
